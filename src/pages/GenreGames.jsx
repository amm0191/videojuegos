...
function GenreGames() {
  ...
  useEffect(() => {
    async function fetchGamesByGenre() {
      setLoading(true)
      try {
        const data = await getGamesByGenre(id, currentPage, pageSize)
        setGames(data.results)
        setTotalPages(Math.ceil(data.count / pageSize))

        // Set genre name from the first game if available
        if (data.results.length > 0) {
          const genre = data.results[0].genres.find((g) => g.id.toString() === id)
          if (genre) {
    ...
    <div className="bg-gradient-to-r from-amber-300 to-green-400 min-h-screen text-white">
      <div className="container mx-auto p-6">
        <h1 className="text-4xl font-extrabold text-center mb-4 drop-shadow-lg">
          Juegos del género: {genreName || `#${id}`}
        </h1>
